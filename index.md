...
203 - Non Authorative Information. The server is a transforming proxy (e.g. a Web accelerator) that received a 200 OK from its origin, but is returning a modified version of the origin's response.
...
### 1xx Codes
Yes, they do exist, however there are only **four** of them, so they're not that hard to remember.
...
### "Unofficial" Codes
The following codes are not specified by any standard. So basically, they are not implemented in for example Express or Apache.
...
498 - Token Invalid. Returned by ArcGIS for Server. Code 498 indicates an expired or otherwise invalid token.

499 - Token required. Returned by ArcGIS for Server. Code 499 indicates that a token is required but was not submitted.

509 - Bandwith limit exceeded. The server has exceeded the bandwidth specified by the server administrator; this is often used by shared hosting providers to limit the bandwidth of customers.

529 - Site overloaded. Used by Qualys in the SSLLabs server testing API to signal that the site can't process the request.

530 - Site frozen. Used by the Pantheon web platform to indicate a site that has been frozen due to inactivity.
...
497 - HTPP Request sent to HTTPS Port. An expansion of the 400 Bad Request response code, used when the client has made a HTTP request to a port listening for HTTPS requests.
...
524 - A Timeout occured. Cloudflare was able to complete a TCP connection to the origin server, but did not receive a timely HTTP response.

525 - SSL Handshake Required. Cloudflare could not negotiate a SSL/TLS handshake with the origin server.
...
Have you ever wondered, how to make a web server that is going to be hosted on your local computer? In this tutorial, I will explain how.

ERROR; Unreachable - the post hasn't been published yet. Sorry for misunderstanding!
